(function() {
	...
	this.create = function(data, dom) {
		...
		if(dataName) {
			...
			var head_height = d.get("head_height");
			var padding = 40;
			var foot_height = padding ;
			foot_height+=25;

			var svg = d3Root.append("svg")
				.attr("width", d.get("width") )
				.attr("height", d.get("height") );
			svg.append("g")
				.append("rect")
				.attr("x",0)
				.attr("y",0)
				.attr("width",d.get("width") || 600)
				.attr("height",d.get("height") || 400)
				.style("fill","#FFF")
				.style("stroke-width",2)
				.style("stroke","#E7E7E7");
			if(d.get("title")) {
			...
			if(d.get("data")) {
				...
				var xScale = D3.scale.linear()
					.domain([0,x.length-1])
					.range([padding,d.get("width") -padding]);
				//定义横轴
				var xAxis = D3.svg.axis()
		            .scale(xScale)			
		            .orient("bottom")
			    ...
			    var xBar=svg.append("g")
			            .attr("class","axis")
			            .attr("transform", "translate(0," + (d.get("height") - foot_height) + ")")
			            .call(xAxis);
			    //通过编号获取对应的横轴标签	

				xBar.selectAll("text")
					.text(function(d){return x.get(d);});
			    function getMax(array) {
			    	var maxdata = 0;
			    	array.forEach(function(o) {
			    		o.get("data").forEach(function(value) {
			    			maxdata=D3.max([maxdata,D3.max(value)]);	
			    		});
			    	});
			    	return maxdata;
			    }
			    var linesData = d.get("data.lines");
			    var maxdata = getMax(linesData);
			    //纵坐标轴比例尺
				var yScale = D3.scale.linear()
					.domain([0,maxdata])
					.range([d.get("height") - foot_height,head_height]);        
				//定义纵轴
				var yAxis = D3.svg.axis()
			            .scale(yScale)
			            .orient("left")
				...
				//添加折线
				var lines=[];
				for(var i=0;i<d.get("data.lines").length;i++)
				{
					var newLine=new CrystalLineObject();
					newLine.init(i);
					lines.push(newLine);		
				...
				{				
					...
					{			
						...
						//添加折线
						this.path=this.group.append("path")
							.attr("d",line(arr.toJSON()))				
							.style("fill","none")
							.style("stroke-width",1)
							.style("stroke",linesData.get(id+".color"))
							.style("stroke-opacity",0.9);
						...
						.attr("r",5)
						.attr("fill",linesData.get(id+".color"));
						this.oldData=arr.toJSON();
					};
					...
					{	
						...
						//路径初始化
						this.path.attr("d",line(arr.toJSON()));			
					...
					this.reDraw=function(id,_duration)
					{
						...
						//路径动画
						this.path.transition().duration(_duration).attr("d",line(arr.toJSON()));
